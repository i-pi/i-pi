...
#print('regtests_folder is: ', regtests_folder)
...
def test_regtest(regtest):
    ...
    if regtest[1]["socket_mode"] == "unix":
        cmd2.append(call_driver + " -m {} -h {} -u ".format(regtest[1]["model"], regtest[1]["address_name"]))
    elif t["socket_mode"] == "inet":
        cmd2.append(call_driver + " -m {} -h {} -p {}".format(regtest[1]["model"], regtest[1]["address_name"], regtest[1]["port_number"]))
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--folder", type=str, default=None, help="Folder of the regressions to test"
    )
    parser.add_argument(
        "--test_all", action="store_true", help="Shall we test all of the regression-examples ?"
    )
