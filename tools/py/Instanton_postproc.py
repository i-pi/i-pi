    ...
    if case == "reactant":
        ...
        if mode == "rate":
            ...
            if not quiet:
                ...
                logQvib = (
                    -np.sum(
                        np.log(betaP * hbar * np.sqrt(np.absolute(np.delete(d, 1))))
                    )
            ...
            action1 = (
                (2 * pots.sum() * factor - nbeads * V0) * 1.0 / (temp * nbeads * kb)
            )
            ...
            if quiet:
                ...
                print(
                    (
                        "We can provied only Tetaphi which value is {} a.u. ".format(
                            tetaphi
                        )
                    )
                )
