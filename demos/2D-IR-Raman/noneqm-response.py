...
class ResponseFunction():
    ...
    def __init__(self, input_param):
        ...
        self.out_name = input_param.out_name + '_' if input_param.out_name is not None else ''
    ...
    def process(self):
        ...
        first = self.chk_first * self.step_traj if self.chk_first is not None else self.step_traj
        last = (self.chk_last + 1) * self.step_traj if self.chk_last is not None else len(x[0])
        for i in range(first, last, self.step_traj): #Start at the first available checkpoint from eq dynamics.
            if i + 1 >= i_step: #Check if we have enough dynamics to go backward -t1.
                #Backward equilibrium trajectory, first operator.
                eq = np.flip(x[0][i - i_step + 1 : i + 1]) #Equilibrium dynamics part from time t (index i) to t-t1.
...
if __name__ == '__main__':
    ...
    parser.add_option('-b', '--op', dest='op', type="str", default='dip, pol',
                      help='Two operators (dip or pol) in the form: <op1>, <op2>.')
    parser.add_option('-p', '--field_pol', dest='field_pol', type="str", default='2, 8',
    ...
    parser.add_option('-o', '--out_name', dest='out_name', type="str", default=None,
